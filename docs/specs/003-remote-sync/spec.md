...
- **FR-322** 每轮同步读取本地文件内容计算 MD5 与 SHA-256 时必须一次流式读取同时更新两个摘要；
  基线写入复用当轮仍有效的本地摘要，避免对未变化附件重复完整读取。大内容始终以流形式处理。
- **FR-323** 每个本地 Vault 必须以稳定授权身份独立保留一个 Google Drive 绑定，包含账号身份、
